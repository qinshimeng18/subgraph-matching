...
def read_focus(q_focus,result_queue,g,q):
    print 'read_focus on'
    ...
    print 'read_focus down'

def mutil_main(graph_path='data.json', query_graph='graph1', k=1, filterFlag=1, commend=1):
    u0 = '1'
    graph_focus = []
    if commend == 1:  # 命令行运行
        k = 1  # 自定义
    ...
    p1 = Process(target=read_focus,args=(q_focus,result_queue,g,q))
    p2 = Process(target=read_focus,args=(q_focus,result_queue,g,q))
    p3 = Process(target=read_focus,args=(q_focus,result_queue,g,q))
    p4 = Process(target=read_focus,args=(q_focus,result_queue,g,q))
    p1.start()
    p2.start()
    p3.start()
    p4.start()
...
if __name__ == '__main__':
    ...
    print(timeit.timeit("mutil_main(graph_path ='data.json',query_graph ='graph1',k =1,filterFlag = 1)",
                        setup="from __main__ import mutil_main", number=1))
    # f.close()
